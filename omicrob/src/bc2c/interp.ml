...
let value_lessequal v1 v2 =
  match v1, v2 with
  | Int i1, Int i2 -> i1 <= i2
  | Int32 i1, Int32 i2 -> (Int32.compare i1 i2) <= 0
  | Int64 i1, Int64 i2 -> (Int64.compare i1 i2) <= 0
  | Nativeint i1, Nativeint i2 -> (Nativeint.compare i1 i2) <= 0
  | Float x1, Float x2 -> x1 <= x2
  | Float_array (_, xs1), Float_array (_, xs2) -> xs1 <= xs2
  | Bytes (_, b1), Bytes (_, b2) -> b1 <= b2
  | _ -> false
...
let ccall arch ooid prim args =
  ...
  | "caml_lessequal", [ v1; v2 ] -> if value_lessequal v1 v2 then Int 1 else Int 0
  | _ ->  
    begin
      match prim with
      ...
      | "caml_random_init" | "caml_random_bits" | "caml_random_bool" | "caml_ensure_stack_capacity" -> ()
      | _ -> 
        let print_arg arg =
          let buf = Buffer.create 16 in
